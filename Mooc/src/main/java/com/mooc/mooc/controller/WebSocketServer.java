...
import com.mooc.mooc.vo.MessageVO;
...
public class WebSocketServer {
    ...
    private Set<UserInfo> createOnlineUsers(ConcurrentHashMap<String,WebSocketServer> webSocketMap){
        Set<UserInfo> users = new HashSet<>();
        Set<String> keys = webSocketMap.keySet();
        for(String key:keys){
            //根据用户名查询用户信息
            UserInfo userInfo = userService.selByUsername(key);
            users.add(userInfo);
        }
        return users;
    }
